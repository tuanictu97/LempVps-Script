#!/bin/bash
#######################################################
# LempVPS Script v2.1 for CentOS 7
# To install type: 
# curl -sO https://sys.ufaceplus.com/scripts/$(rpm -E %centos)/install && bash install
#######################################################
lempvps_version="2.1"
...
extplorer_id="82"
script_root="https://raw.githubusercontent.com/tuanictu97/LempVps-Script/master/scripts"
script_url="https://raw.githubusercontent.com/tuanictu97/LempVps-Script/master/scripts/7"
...
read server_name
if [ "$server_name" = "" ]; then
	server_name="ufaceplus.com"
	echo "Ban nhap sai, he thong dung ufaceplus.com lam ten mien chinh"
fi
...
cat > "/etc/php.d/00-lempvps-custom.ini" <<END
...
<hr><center>lempvps-nginx</center>
</body>
</html>
...
server {
	...
	auth_basic_user_file /home/$server_name/private_html/lempvps/.htpasswd;
...
mv -f administrator/* .
rm -rf administrator
printf "admin:$(openssl passwd -apr1 $admin_password)\n" > /home/$server_name/private_html/lempvps/.htpasswd
sed -i "s/rootpassword/$root_password/g" /home/$server_name/private_html/lempvps/SQLManager.php
...
cat > "/etc/lempvps/scripts.conf" <<END
lempvps_version="$lempvps_version"
server_name="$server_name"
server_ip="$server_ip"
...
END
chmod 600 /etc/lempvps/scripts.conf

clear
printf "=========================================================================\n"
printf "Cau hinh hoan tat, bat dau them menu lempvps, nhanh thoi... \n"
printf "=========================================================================\n"

wget -q $script_url/lempvps -O /bin/lempvps && chmod +x /bin/lempvps
mkdir /etc/lempvps/menu/
cd /etc/lempvps/menu/
wget -q $script_url/component/menu.zip
unzip -q menu.zip && rm -f menu.zip
chmod +x /etc/lempvps/menu/*

clear
cat > "/root/lempvps-script.txt" <<END
=========================================================================
                           MANAGE VPS INFORMATION                        
=========================================================================
Lenh truy cap menu LempVPS Script: lempvps

Domain chinh: http://$server_name/ hoac http://$server_ip/

LempVPS Script Admin:	http://$server_name:$admin_port/ hoac http://$server_ip:$admin_port/
...
chmod 600 /root/lempvps-script.txt

if [ "$1" = "wordpress" ]; then
	printf "=========================================================================\n"
	printf "Hoan tat qua trinh cai dat LempVPS Script + WordPress! \n"
	printf "=========================================================================\n"
	printf "Tiep theo ban hay truy cap http://$server_name \n hoac http://$server_ip de cau hinh WordPress \n"
else
	printf "=========================================================================\n"
	printf "Scripts LempVPS da hoan tat qua trinh cai dat... \n"
	printf "=========================================================================\n"
...
printf "=========================================================================\n"
printf "LempVPS Script Admin: http://$server_name:$admin_port/ \n hoac http://$server_ip:$admin_port/\n\n"
...
printf "=========================================================================\n"
printf "Thong tin quan ly duoc luu tai: /root/lempvps-script.txt \n"
printf "=========================================================================\n"
printf "***Luu y: Port dang nhap SSH da duoc doi tu 22 sang 2222 de bao mat VPS\n"
printf "=========================================================================\n"
printf "De quan ly server, ban hay dung lenh \"lempvps\" khi ket noi SSH.\n"
printf "=========================================================================\n"
