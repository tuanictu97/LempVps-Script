...
import (
	...
	"io.github.tuanictu97/api/pkg/errors"
...
func RateLimiterWithConfig(config RateLimiterConfig) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			logger.Context(ctx).Error("Rate limiter middleware error", zap.Error(err))
			utilx.ResError(c, errors.InternalServerError(errors.ErrInternalServerErrorID, "Internal server error, please try again later."))
		} else if allowed {
			c.Next()
		} else {
			utilx.ResError(c, errors.TooManyRequests(errors.ErrTooManyRequestsID, "Too many requests, please try again later."))
		}
	}
}
...
func (s *RateLimiterRedisStore) Allow(ctx context.Context, identifier string, period time.Duration, maxRequests int) (bool, error) {
	...
	result, err := s.limiter.AllowN(ctx, identifier, redis_rate.PerSecond(int(period.Seconds())), maxRequests)
	if err != nil {
		return false, err
	}
